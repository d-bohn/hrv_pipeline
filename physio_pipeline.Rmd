...
archtyical coding "rule of three"). However, since its inception, bits and peices of this
...
anaylze, and store data, but due to the nature of physiological data cleaning and analysis,
...
Because the functions written for this pipeline are desgined for this specific
...
I was able to ammend `data.table::fread` to read compressed
text files, as well as some inline "hacks" to output compressed text files.
This significantly reduces the diskspace required for the project. Nonetheless,
...
attempts to impute a datapoint via `forecast::tsclean`. It also saves a "before and after"
...
### Make Trigger Events
Next, I have to extract the appropraite triggers from the raw data and coerce them
into a format that is acceptable for downstream analysis and plotting (i.e., `RHRV`).

The function saves the relevant triggers and timestamps to a compressed `.RData` file.
...
the appropriate functions in the `RHRV` package. Specfically, it performs a spectral power band analysis, 
...
dataframes to the number of rows in our joined dataset. If the join is considered
